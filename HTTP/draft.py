"""
    ...
    HTTP — это протокол прикладного уровня передачи данных, сначала — в виде
    гипертекстовых документов в формате HTML, теперь в поизвольном виде.

    По большому счету это просто соглашение о формате текстовых сообщений, между сервером и клиентом

    Протокол текстовый, но сокеты у нас двоичные.

    Простыми словами:
    HTTP протокол - это набор правил, описывающий обмен данными между приложениями(клиентом и сервером).
    Чтобы разные приложения в сети могли общаться друг с другом, понимать друг друга, им нужны общие соглашения.
    Если общих соглашений не будет, то каждое приложение будет придерживаться своих правил и не сможет общаться
    ...
    использования механизмов управления соединением.

    HTTP протокол также касается использования механизмов кэширования, сжатия, аутентификации, безопасности и других
    аспектов обмена данными. Эти механизмы позволяют повысить эффективность, надежность и защищенность передачи данных
    между клиентом и сервером.

    HTTP протокол не только передает гипертекстовые документы в формате HTML, но и другие виды данных, такие как изображения,
    аудио, видео, JSON, XML и т.д. Для того, чтобы определить тип и характеристики передаваемых данных, HTTP протокол
    использует заголовки Content-Type и Content-Encoding .

    HTTP пакет состоит из трёх частей:
    1 Стартовая строка

        В стартовой строке ЗАПРОСА указываются метод запроса, URI, версия
        протокола.
        ====================
        ...
        ====================
        Аргументы GET запроса передаются в URI
